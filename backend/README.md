...
| POST   | `/predict/suggestions`| Get personalized lifestyle suggestions using AI |
| POST   | `/predict/chat`       | Chat with an AI health coach about results      |
...
Provides 3-5 prioritized lifestyle suggestions generated by Llama 3.3 using the patient profile and both CVD models' predictions.
...
Enables interactive follow-up questions with a health chatbot that has full context on the patient's profile and model predictions.
...
curl -s -X POST http://localhost:8000/predict/chat \
  ...
  -d '{
    ...
    "history": [],
    "message": "What is the most important thing I can do to lower my blood pressure?"
  }' | python -m json.tool
```
...
```json
{
    "reply": "Your systolic blood pressure is 148, which is higher than the recommended range. Increasing your physical activity by going from 1 day a week to at least 3-4 days..."
}
```
